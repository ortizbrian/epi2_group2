...
# 2023-11-08: NHANES data imported with 7122 rows and 103 columns. 
#  Data last modified:  2023-11-08 17:24:31 
```
...
```{r}
hist(included_data$active_mod_rec)
```

Boxplot demonstrating significant right skew, with many outliers to the far positive ranges of values

```{r}
boxplot(included_data$active_mod_rec)
```

QQ demonstrating significant deviation in the far positive range of values.

```{r}
qqnorm(included_data$active_mod_rec)
qqline(included_data$active_mod_rec)
```
