...
-   The two race columns were checked, and found to have different categories
...
-   Subjects with any exclusion (or lack of inclusion) criteria were flagged for exclusion in `drop_flag`
  ...
  mutate(
    ins_govt = ifelse( 
      (ins_govt_medicare | ins_govt_medigap | ins_govt_medicaid |
         ins_govt_chip | ins_govt_state | ins_govt_other), TRUE, NA
      )
    )
...
nhanes_data |>
  ...
  filter(n_ins > 1) |>
  select(id, n_ins, ins_govt, ins_private, ins_none, ins_govt_medicaid, ins_govt_medicare, ins_govt_medigap, ins_govt_chip, ins_govt_state, ins_govt_other)
```
...
nhanes_data <- nhanes_data |>
  ...
  mutate(
    alc_heavy = case_when(
      sex == "Female" & alc_per_week >=8 ~ TRUE,
      sex == "Female" & alc_per_week <8 ~ FALSE,
      sex == "Male" & alc_per_week >=15 ~ TRUE,
      sex == "Male" & alc_per_week <15 ~ FALSE,
      TRUE ~ alc_heavy
    )
  ) |>
  # Override if per-day is over threshold, regardless of per-week
  mutate(
    alc_heavy = case_when(
      sex == "Female" & alc_per_day >= 4 ~ TRUE,
      sex == "Female" & alc_per_day < 4 ~ FALSE,
      sex == "Male" & alc_per_day >= 5 ~ TRUE,
      sex == "Male" & alc_per_day < 5 ~ FALSE,
      TRUE ~ alc_heavy
    )
  ) |>
...
## Verifying Teaching Team Variable
...
We isolated the columns that were subject-specific, and took only a single row for each subject.
...
We further wished to exclude anyone who had not endorsed ever being told they needed to take a prescription medication for their cholesterol. This resulted in an additional 5,290 rows excluded.

```{r}
combined_data <- combined_data |>
  mutate(drop_flag = case_when(
    !is.na(cholesterol_rx_ever) ~ TRUE,
    !cholesterol_rx_ever ~ TRUE,
    TRUE ~ drop_flag
    )
  )
...
This has left 1,831 subjects for analysis. Due to this smaller size, the excluded rows have not been dropped, but the flag kept in the data set.

```{r}
attributes(combined_data[['drop_flag']])$label <- paste(
  "Flag for exclusion: age < 20, cancer history, taking insulin or other",
  "glucose lowering medications, or never told a lipid-lowering medication",
  "was needed"
)
